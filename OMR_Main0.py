...
def execute():
    ...
    try:
        ...
        # cv2.imshow("IMGBOGCOUNTOUR",imgBigContour)
        ...
        # roll_no_Points=utlis.getCornerPoints((rectCon[4]))  # GET CORNER POINTS OF THE ROLL NO RECTANGLE
        ...
        if biggestPoints.size != 0 and gradePoints.size != 0:
            ...
            imgGradeDisplay = cv2.warpPerspective(img, matrixG, (225, 75)) # APPLY WARP PERSPECTIVE
            # cv2.imshow("Kedar",imgGradeDisplay)

            cv2.drawContours(imgBigContour, roll_no_Points, -1, (0, 0, 255), 20)  # DRAW THE BIGGEST CONTOUR
            roll_no_Points = utlis.reorder(roll_no_Points)  # REORDER FOR WARPING
            ptsr1 = np.float32(roll_no_Points)  # PREPARE POINTS FOR WARP
            # print(ptsr1)
            ptsr2 = np.float32([[0,0], [100, 0], [0,35], [100,35]])  # PREPARE POINTS FOR WARP
            matrixR = cv2.getPerspectiveTransform(ptsr1, ptsr2)  # GET TRANSFORMATION MATRIX
            imgRollDisplay = cv2.warpPerspective(img, matrixR, (100, 40))  # APPLY WARP PERSPECTIVE
            cv2.imshow("Roll_No",imgRollDisplay)

            # APPLY THRESHOLD
            imgWarpGray = cv2.cvtColor(imgWarpColored,cv2.COLOR_BGR2GRAY) # CONVERT TO GRAYSCALE
            imgThresh = cv2.threshold(imgWarpGray, 170, 255,cv2.THRESH_BINARY_INV )[1] # APPLY THRESHOLD AND INVERSE
            boxes = utlis.splitBoxes(imgThresh) # GET INDIVIDUAL BOXES
            cv2.imshow("Split Test ", boxes[4])
            countR=0
            countC=0
            ...
            cv2.imshow('K',imgRawGrade)

            # NEW BLANK IMAGE WITH GRADE AREA SIZE
            cv2.putText(imgRawGrade,str(int(score))+"%",(70,50)
                        ,cv2.FONT_HERSHEY_COMPLEX,2,(102,0,0),3) # ADD THE GRADE TO NEW IMAGE
            cv2.imshow('New', imgRawGrade)
            ...
            # cv2.imshow('New', imgInvGradeDisplay)
            imgFinal = cv2.addWeighted(imgFinal, 1, imgInvGradeDisplay, 1,0)
            cv2.imshow('FinalKed',imgFinal)

            # IMAGE ARRAY FOR DISPLAY
            imageArray = ([img,imgGray,imgCanny,imgContours],
                        [imgBigContour,imgThresh,imgWarpColored,imgFinal])
            cv2.imshow("Final Result2333", imgFinal)
    except:
        imageArray = ([img,imgGray,imgCanny,imgContours],
    ...
    # cv2.imshow('Result1', stackedImage)
    cv2.waitKey(10000)
    count += 1
    return score
